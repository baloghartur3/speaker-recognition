...
def concatenate(args,lines):
	...
	for line in lines:
		...
		#out 	= subprocess.call('type %s/%s > %s/%s' %(args.save_path,infile,args.save_path,outfile), shell=True)
		out 	= subprocess.call('type %s > %s' %(infile,outfile), shell=True)
		#out 	= subprocess.call('type  > %s' %(infile,outfile), shell=True)
